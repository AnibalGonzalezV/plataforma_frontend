...
export default function CourierOrdersList() {
  // TODO: Obtener los pedidos asignados al repartidor desde la API
  // const { data: orders, isLoading } = useQuery(['courierOrders'], getCourierOrders);
  ...
  const handleDelivered = (orderId: number) => {
    // TODO: Llamar a la API para marcar como entregado
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-4">
      <div className="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl border border-gray-700 p-8">
        <h2 className="text-2xl font-bold mb-6 text-white text-center flex items-center justify-center gap-2">
          <Truck className="h-6 w-6 text-yellow-400" /> Pedidos para entregar
        </h2>
        <div className="space-y-4">
          {orders.length === 0 ? (
            <div className="flex flex-col items-center py-12">
              <Package className="h-12 w-12 text-gray-600 mb-4" />
              <p className="text-gray-400 text-center">No tienes pedidos asignados.</p>
            </div>
          ) : orders.map(order => (
            <div key={order.id} className="bg-gray-900 border border-gray-700 rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2 shadow-lg">
              <div>
                <div className="text-white font-semibold text-lg flex items-center gap-2">
                  <MapPin className="h-5 w-5 text-yellow-400" /> {order.address}
                </div>
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <button onClick={() => handleDelivered(order.id)} className="mt-2 px-4 py-1 rounded-lg bg-green-600 hover:bg-green-700 text-white text-sm font-medium transition">Marcar como entregado</button>
              </div>
            </div>
          ))}
        </div>
